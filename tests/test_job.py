...
class FakeKJobOld(PBSJob):
    def _run_qstat_to_get_full_job_attributes(self):
        stand_in_output = [
            ...
            "resources_used.mem = 1528kb",
            "resources_used.ncpus = 16",
            "resources_used.vmem = 15936kb",
            ...
            "exec_host = k3ar5n1/0*16",
            "exec_vnode = (k3ar5n1:ncpus=16)",
            "Hold_Types = n",
            "Join_Path = oe",
            "Keep_Files = n",
            "Mail_Points = a",
            ...
            "Priority = 0",
            "qtime = 1649348639 (Thu Apr 07 12:23:59 EDT 2022)",
            "Rerunable = False",
            "Resource_List.mem = 31gb",
            "Resource_List.mpiprocs = 16",
            "Resource_List.ncpus = 16",
            "Resource_List.nodect = 1",
            "Resource_List.nodegroup = K3a-open",
            "Resource_List.place = scatter:excl",
            "Resource_List.select = 1:ncpus=16:mpiprocs=16",
            "Resource_List.walltime = 72:00:00",
            "stime = 1649348640 (Thu Apr 07 12:24:00 EDT 2022)",
            "session_id = 22053",
            "jobdir = /u/kejacob1",
            "substate = 92",
...
def test_read_K_properties_from_qstat():
    ...
    assert (
        job.workdir
        == "/lustre3/hpnobackup2/kejacob1/projects/rca/buffet/cases/oat15a/ncfv_rans_pointwise_revised_grid_cc/grid_l6/aoa3.6/steady"
    )
    assert job.model == ""
...
def test_read_K_old_properties_from_qstat():
    ...
    assert (
        job.workdir
        == "/lustre3/hpnobackup2/kejacob1/projects/cad_to_solution/pbs4py/examples/batch_with_job_limit/sample0"
    )
    assert job.model == ""
...
def test_K_mtime_parsed():
    job = FakeKJob("2493761")
    assert job.mtime == datetime(2024, 10, 24, 12, 26, 32)
    assert "Thu Oct 24" in job.mtime_raw


def test_K_old_mtime_parsed_from_epoch():
    job = FakeKJobOld("2493765")
    assert job.mtime == datetime.fromtimestamp(1649348653)


def test_NAS_mtime_parsed_from_epoch():
    job = FakeNASJob("13198744")
    assert job.mtime == datetime.fromtimestamp(1649355753)


def test_unknown_job_mtime_none():
    job = FakeUnknownJob("123456")
    assert job.mtime is None
    assert job.mtime_raw == ""
