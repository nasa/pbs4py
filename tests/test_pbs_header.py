...
def test_job_dependency_line_of_header(pbs_header_test: PBS):
    ...
    assert [
        "#PBS -W depend=afterok:a.1234"
    ] == pbs_header_test._create_job_dependencies_header_line(dependency="a.1234")

    pbs_header_test.dependency_type = "before"
    assert ["#PBS -W depend=before:b.4321"] == pbs_header_test._create_job_dependencies_header_line(
        dependency="b.4321"
    )
